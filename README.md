...
```bash
node dist/main.js setup init --data-dir /absolute/path/to/dalil-data
```
...
```bash
node dist/main.js setup config set openai.key
```
...
```bash
node dist/main.js runner tui --mode managed
```

`runner tui`는 Runner를 시작하고 TUI를 엽니다. Runner만 백그라운드처럼 유지하려면 `runner start`를 사용합니다.

### 6) 다른 터미널에서 필드 작성

```bash
node dist/main.js runner fields list
...
| 그룹 | 명령어 | 설명 |
|---|---|---|
| Setup | `dalil setup init --data-dir <path>` | 데이터 디렉토리 초기화 및 전역 config 등록 |
| Setup | `dalil setup config set openai.key` | OpenAI API 키 저장(입력 프롬프트) |
| Setup | `dalil setup doctor` | 환경 점검(경로 권한, 키 설정, Playwright, 편집기 등) |
| Runner | `dalil runner start [--mode managed/attach] [--cdp <url>] [--port <n>]` | Runner 시작(daemon) |
| Runner | `dalil runner tui [--mode managed/attach] [--cdp <url>] [--port <n>]` | Runner 시작 + TUI 진입 |
...
| Fields | `dalil runner fields list [--format table/json]` | 현재 페이지 입력 필드 스캔 결과 조회 |
| Fields | `dalil runner fields show <fieldId>` | 필드 상세 메타데이터 확인 |
| Fields | `dalil runner fields highlight <fieldId>` | 브라우저에서 필드 하이라이트 |
...
| Revert | `dalil runner revert <fieldId>` | 마지막 반영 내용 롤백 |
...
- `runner start`/`runner tui` 명령은 Playwright가 필요합니다: `npm i playwright`
